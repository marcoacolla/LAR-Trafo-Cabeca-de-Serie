...
class Curvature:
    ...
    def computeICR(self, angle_offset=0):
        ...
        if self.vehicle.curve_mode in ["curve", "pivotal"]:
            ...
            cx, cy = self.vehicle.getCameraRelativePosition()  # centro visual do robô
    ...
    def update(self, surface):
        ...
        vehicle_center = self.vehicle.getCameraRelativePosition()  # posição visual
        ...
        pygame.draw.circle(self.surface, self.base_color,
                           (int(icr[0]), int(icr[1])), main_radius, 1)

        # ponto do ICR
        pygame.draw.circle(self.surface, self.base_color,
                           (int(icr[0]), int(icr[1])), 5)
