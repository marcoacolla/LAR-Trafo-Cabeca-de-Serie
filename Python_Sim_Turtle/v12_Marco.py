...
def main():
    # Variável para controlar a duração da transição
    transition_duration = [10]  # lista para mutabilidade

    turtle.tracer(0, 0)
    is_transitioning = False

    # Controle: 'can' ou 'teclado'
    control_mode = ["can"]  # lista para mutabilidade
    ...
    def toggle_control_mode():
        if control_mode[0] == "can":
            control_mode[0] = "teclado"
            transition_duration[0] = 100
            print("Controle: TECLADO (WASD, espaço)")
        else:
            control_mode[0] = "can"
            transition_duration[0] = 10
            print("Controle: CAN (joystick)")
    print("Pressione TAB para alternar entre controle CAN e TECLADO.")
    def setMode(mode, curveStart=0, duration=None):
        ...
        def on_completion():
            if new_mode == "pivotal":
                ...
                return
        if duration is None:
            duration = transition_duration[0]
        if new_mode in ["curve", "diagonal", "pivotal"]:
            smoothSteeringTransition(new_mode, target_angle=plataforma.angle_offset, on_complete=on_completion, duration=duration)
        else:
            smoothSteeringTransition(new_mode, duration=duration)
    ...
    def joystickAngleOffsetUpdate():
        if control_mode[0] == "can":
            ...
            plataforma.curvature.update(angle_offset=plataforma.angle_offset)
            # Atualiza a "câmera" para centralizar o robô ou o ICR em todos os frames
            if plataforma.curve_mode in ["curve", "pivotal"] and plataforma.icr_global is not None:
    ...
    def updateAngleSpeed(key):
        ...
        if plataforma.angle_offset > 220 or plataforma.angle_offset < -220:
            setMode("straight", duration=transition_duration[0])
        else:
            plataforma.curvature.update(angle_offset=plataforma.angle_offset)
        turtle.update()
    ...
    def callMovement(direction, step=5.0):
        ...
        plataforma.curvature.update(angle_offset=plataforma.angle_offset)
        ui.clear_break()
        # Durante transição de modo, sempre centraliza no centro do robô para evitar salto
        if is_transitioning:
            screen.update_camera(*plataforma.getPosition())
        else:
            if plataforma.curve_mode in ["curve", "pivotal"] and plataforma.icr_global is not None:
    ...
    turtle.listen()
    def teclado_callMovement(direction):
        if control_mode[0] == "teclado":
            callMovement(direction)
    def teclado_toggleMode():
        if control_mode[0] == "teclado":
            toggleMode()
    def teclado_ResetVehicle():
        if control_mode[0] == "teclado":
            ResetVehicle()
    def teclado_keyPressed_A():
        if control_mode[0] == "teclado":
            keyPressed_A()
    def teclado_keyReleased_A():
        if control_mode[0] == "teclado":
            keyReleased_A()
    def teclado_keyPressed_D():
        if control_mode[0] == "teclado":
            keyPressed_D()
    def teclado_keyReleased_D():
        if control_mode[0] == "teclado":
            keyReleased_D()
    def teclado_keyPressed_Q():
        if control_mode[0] == "teclado":
            keyPressed_Q()
    def teclado_keyPressed_E():
        if control_mode[0] == "teclado":
            keyPressed_E()

    turtle.onkeypress(lambda: teclado_callMovement("forward"), "w")
    turtle.onkeypress(lambda: teclado_callMovement("backward"), "s")
    turtle.onkeypress(teclado_toggleMode,  "space")
    turtle.onkeypress(teclado_ResetVehicle,"r")
    turtle.onkeypress(teclado_keyPressed_A, "a")
    turtle.onkeyrelease(teclado_keyReleased_A, "a")
    turtle.onkeyrelease(ui.draw_break, "w")
    turtle.onkeyrelease(ui.draw_break, "s")
    turtle.onkeypress(teclado_keyPressed_D, "d")
    turtle.onkeyrelease(teclado_keyReleased_D, "d")
    turtle.onkeypress(teclado_keyPressed_Q, "q")
    turtle.onkeypress(teclado_keyPressed_E, "e")
    turtle.onkeypress(toggle_control_mode, "Tab")
